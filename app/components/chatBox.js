...
class ChatBox extends React.Component {
  ...
  static generateMessage( from, date, text, right ) {
    return (
      <Message key={from + date + Math.random()}
               name={from}
               date={date}
               text={text}
               right={right} />
  ...
  sendMessage() {
    let text = this.state.textToSend;
  ...
  keyPress( event ) {
	...
	static makeDuration( record ) {
		let curDur = record.currentStateDuration;
      ...
      return '';
		...
		let parts = /(?:([0-9])\.)?([0-9]{2}):([0-9]{2}):([0-9]{2})?/.exec( curDur ),
      d = parseInt( parts[1] ),
		  h = parseInt( parts[2] ),
		  m = parseInt( parts[3] ),
		  s = parseInt( parts[4] ),
      ms = (d ? d * 24 * 60 * 60 * 1000 : 0) + h * 60 * 60 * 1000 + m * 60 * 1000
        + s * 1000;
			
		let dur = new Date() - new Date( record.dateTime ) - ms;
    ...
    if(new Date() - new Date( record.dateTime ) < 24 * 60 * 60 * 1000) {
      if(record.isIgnitionOn !== undefined) {
        state = record.isIgnitionOn
          ? (record.speed < 4 ? 'Idling' : 'Driving')
          : 'Stopped'
      } else {
	...
	render() {
		const stat = this.props.currentUserStatus || {
        record: {
		...
		return (
  		<div className="chat">
             ...
             style={{display: 'flex'}}>
  			...
  			<div ref={(ref) => this.chatHistory = ref}
          ...
          <div className="bottomOptions">
            ...
            <div className="insert"
              ...
              <img style={{cursor:'pointer',padding:'0 10px'}}
                   src="https://www.google.com/images/branding/product/ico/maps_32dp.ico"
                   onClick={ () => { this.setState( { showGoogleMap: !this.state.showGoogleMap } ) } } />
              {this.state.showGoogleMap ? (
                <div style={{padding: '0 0 10px 0'}}>
                  <input type="text"
                         placeholder="Address"
                         value={this.state.googleMapAddress}
                         onChange={ ( evt ) => this.setState( {googleMapAddress: evt.target.value } ) } />
                  <button style={{float: 'none',padding:'0 10px'}}
                          onClick={ () => {
                  ...
                  const y = parseFloat(resp[0].y).toFixed(4)
								...
								this.textArea.value += googleURL; } }>Geocode</button>
                </div>
            ) : null}
            </div>
          </div>
